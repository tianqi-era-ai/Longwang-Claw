...
const normalizeAccountId = typeof plugin_sdk_1.normalizeAccountId === 'function'
    ? plugin_sdk_1.normalizeAccountId
...
function getLarkAccountIds(cfg) {
    ...
    const hasDefault = accountIds.some((id) => id.trim().toLowerCase() === plugin_sdk_1.DEFAULT_ACCOUNT_ID);
    if (!hasDefault) {
        const base = baseConfig(section);
        if (base.appId && base.appSecret) {
            return [plugin_sdk_1.DEFAULT_ACCOUNT_ID, ...accountIds];
        }
    }
    return accountIds;
}
...
function getLarkAccount(cfg, accountId) {
    const requestedId = accountId ? (normalizeAccountId(accountId) ?? plugin_sdk_1.DEFAULT_ACCOUNT_ID) : plugin_sdk_1.DEFAULT_ACCOUNT_ID;
    const section = getLarkConfig(cfg);
    if (!section) {
        return {
    ...
    const base = baseConfig(section);
    const accountMap = getAccountMap(section);
    const accountOverride = accountMap && requestedId !== plugin_sdk_1.DEFAULT_ACCOUNT_ID
